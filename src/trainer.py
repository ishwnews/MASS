...
class Trainer(object):
    ...
    def __init__(self, data, params):
        ...
        self.stats = OrderedDict(
            ...
            [('MA-%s' % lang, []) for lang in params.ms_steps] +
...
class EncDecTrainer(Trainer):
    ...
    def mask_word(self, w):
        ...
        if p >= 0.2:
            ...
            return np.random.randint(14, self.params.n_words)
        else:
            return w
    ...
    def mask_interval(self, length):
        mask_length = round(length * self.params.word_mass)
        #mask_start  = np.random.randint(1, length - mask_length + 1)
        mask_start  = self.weight_start(length - mask_length + 1)
        return mask_start, mask_length

    def mask_sent(self, x, l):
        max_len = 0
        starts, inputs, targets, outputs, len1, len2 = [], [], [], [], [], []
        for i in range(l.size(0)):
            words = x[:l[i], i].tolist()
            start, length = self.mask_interval(len(words))

            # For decoder 
            output_i = words[start   : start+length  ].copy()
            target_i = words[start-1 : start+length-1].copy()
            ...
            for j, w in enumerate(words):
                if j >= start and j < start+length:
                    #w = self.drop_word(w)
                    w = self.mask_word(w)
                if w is not None:
                    input_i.append(w)
            input_i[-1] = self.params.eos_index
            inputs.append(input_i)
            len1.append(len(input_i))
        
        x1  = torch.LongTensor(max(len1), l.size(0)).fill_(self.params.pad_index)
        x2  = torch.LongTensor(max_len, l.size(0)).fill_(self.params.pad_index)
        y   = torch.LongTensor(max_len, l.size(0)).fill_(self.params.pad_index)
        pos = torch.LongTensor(max_len, l.size(0))
        l1  = torch.LongTensor(len1)
        l2  = torch.LongTensor(len2)
        for i in range(l.size(0)):
            x1[:len1[i], i].copy_(torch.LongTensor(inputs[i]))
            x2[:len2[i], i].copy_(torch.LongTensor(targets[i]))
            y[:len2[i], i].copy_(torch.LongTensor(outputs[i]))
            pos[:, i].copy_(torch.LongTensor(np.arange(0, max_len) + starts[i]))
    ...
    def ms_step(self, lang, lambda_coeff):
        assert lambda_coeff >= 0
        if lambda_coeff == 0:
            return
        params = self.params
        ...
        x_, len_ = self.get_batch('ms', lang)
        ...
        dec2 = self.decoder('fwd', x=x2, lengths=len2, langs=langs2, causal=True, src_enc=enc1, src_len=len1, positions=positions)
