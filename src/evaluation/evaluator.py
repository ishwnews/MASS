    ...
    def evaluate_mt(self, scores, data_set, lang1, lang2, eval_bleu):
        ...
        for batch in self.get_iterator(data_set, lang1, lang2):
            ...
            if eval_bleu:
                ...
                if len(params.ms_steps) > 0:
                    generated, lengths = decoder.generate(enc1, len1, lang1_id, max_len=max_len)
                    mono_hypothesis.extend(convert_to_text(generated, lengths, self.dico, params))
        ...
        if eval_bleu:
            ...
            # mass prediction step
            if len(params.ms_steps) > 0:
                hyp_name = 'hyp{0}.{1}-{2}.{3}.txt'.format(scores['epoch'], lang1, lang1, data_set)
                hyp_path = os.path.join(params.hyp_path, hyp_name)
                ref_path = params.ref_paths[(lang1, lang2, data_set)]

                # export sentences to hypothesis file / restore BPE segmentation
                with open(hyp_path, 'w', encoding='utf-8') as f:
                    f.write('\n'.join(mono_hypothesis) + '\n')
                restore_segmentation(hyp_path)
